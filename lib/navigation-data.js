	...
	"api-reference": [
		...
		},
		{
			...
			items: [
				...
				{ name: "Speech-to-Text REST API", hasDropdown: false },
			],
		},
	],
	guides: [
...
function formatSearchNavLabel(entry) {
	if (entry.parentName) return `${entry.parentName} › ${entry.itemName}`;
	if (entry.heading) return `${entry.heading} › ${entry.itemName}`;
	return entry.itemName;
}

/**
 * Sections for search modal browse links; `i18nKey` maps to navbar tab keys used with i18next.
 */
export function getGroupedSearchNavigation() {
	const defs = [
		{ i18nKey: "tabs.docs", sectionKey: "docs", baseRoute: "/docs" },
		{
			i18nKey: "tabs.apiReference",
			sectionKey: "api-reference",
			baseRoute: "/api-reference",
		},
		{ i18nKey: "tabs.guides", sectionKey: "guides", baseRoute: "/guides" },
		{
			i18nKey: "tabs.changelog",
			sectionKey: "changelog",
			baseRoute: "/changelog",
		},
		{ i18nKey: "tabs.support", sectionKey: "support", baseRoute: "/support" },
	];

	return defs.map(({ i18nKey, sectionKey, baseRoute }) => {
		const flat = flattenNavigation(navigationData[sectionKey], baseRoute);
		return {
			i18nKey,
			items: flat.map((entry) => ({
				href: entry.route,
				label: formatSearchNavLabel(entry),
			})),
		};
	});
}
